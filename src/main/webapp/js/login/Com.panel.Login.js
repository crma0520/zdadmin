Com.panel.Login=Ext.extend(Ext.Panel,{
		  		  		...
		  		  		'<div class="middle-layout l-box-Systemname"></div>' +
		  		   		'<div class="middle-layout l-box-middle">' +
		  		  			 '<div class="l-box-login">' +
		           				'<div class="l-box-login-title yahei">'+LANGUAGE_PACKAGE['DGS_SYS_LOGIN']+'</div>' +
		           				...
		           				'<div class="l-box-login-btnbox">' +
		          					 '<b class="btn_blue" id="btnLogin"><span class="span-login">'+LANGUAGE_PACKAGE['DGS_SYS_LOGIN_BUTTON']+'</span></b>' +
		          				 '</div>' +
		  		   		...
		  		   		'</div><br/>'+
		  		   		'<div class="middle-layout l-box-bottom yahei">'+LANGUAGE_PACKAGE['DGS_SYS_COMPANY'] + ' ' + VERSION+'</div>' +
		  		   	'</div>' +
		  		  '</div>',
		  	listeners : {
		  		afterlayout : {
	...
	login:function(){
		...
		var dlm = Ext.get('dlm').getValue();
		var mm = Ext.get('mm').getValue();
		var authCode = Ext.get('validateId').getValue();
		if(dlm == '' || dlm == null) {
			Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_TIPS_USERNAME'];
			return;
		}
		if(mm == '' || mm == null) {
			Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_TIPS_PASSWORD'];
			return;
		}
		if(authCode == '' || authCode == null) {
			Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_TIPS_VALIDATE'];
			return;
		}
		var params = {
			account : dlm,
			password : faultylabs.MD5(mm),   // md5加密
			//userAuthority : DGS_CONSTANTS_BACKGROUND_USER,
			pageName : self.pageName,
			validateCode : authCode
		};
		//对于传参需要进行加密传输
		var userInfo = rc4(Str2Utf8(Ext.encode(params)), DGS_CONSTANTS_ENCRYPTEDKEY);
		var myMask  = new Ext.LoadMask({
			msg : LANGUAGE_PACKAGE['DGS_SYS_QUERY_DATA'],//'正在请求数据，请稍候...',
    		target : self
		});
		myMask.show();
		Ext.Ajax.request({
			...
			params : new Base64().encode(userInfo),
			success : function(resp){
				...
				if(data.success){
					...
					if(data.retcode == CONSTANTS_STATE_LOGIN_LOCK){//账号锁定
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_STATE_LOCK'];
					}else if(data.retcode == CONSTANTS_STATE_LOGIN_LOGOUT){//账号注销
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_STATE_LOGOUT'];
					}else if(data.retcode == CONSTANTS_STATE_LOGIN_OVERDUE){//账号过期
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_STATE_OVERDUE'];
					}else if(data.retcode == CONSTANTS_STATE_LOGIN_PWD_ERROR){//密码错误
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_STATE_PWD_ERROR'];
					}else if(data.retcode == CONSTANTS_STATE_LOGIN_NO_ANTHORITY){//没有权限登录
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_NO_ANTHORITY'];
					}else if(data.retcode == CONSTANTS_STATE_LOGIN_NOT_EXIST){//用户名或者密码不存在
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_STATE_NOT_EXIT'];
					}else if(data.retcode == CONSTANTS_STATE_LOGIN_VALIDATE_ERROR){//验证码错误
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_STATE_VALIDATE_ERROR'];
					}else if(data.retcode == CONSTANTS_STATE_LOGIN_PASSWORD_COUNT_ERROR){//密码输错超过5次
						Ext.get("info").dom.innerHTML = LANGUAGE_PACKAGE['DGS_SYS_LOGIN_STATE_PASSWORD_COUNT_ERROR'];
					}else{//系统错误
