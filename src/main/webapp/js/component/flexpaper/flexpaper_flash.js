 ...
 * 
 * @example
 * onExternalLinkClicked("http://www.google.com")
 * 
 * @param String
 *            link
 */
 ...
 * 
 * @example
 * onDocumentLoadedError( "Network error" );
 * 
 * @param String
 *            errorMessage
 */
	...
	var f = extend(window.flashembed, {
		...
		getHTML : function(opts, conf) {
			...
			var html = '<object width="' + opts.width + '" height="'
					+ opts.height + '" id="' + opts.id + '" name="' + opts.id
					+ '"';
	...
	function Flash(root, opts, conf) {
		...
		if (f.isSupported(opts.version)) {
			...
			if (!root.innerHTML.replace(/\s/g, '')) {
				 ...
				 * installed") + "</h3>" +
				 * 
				 * (root.tagName == 'A' ? "<p>Click here to download latest
